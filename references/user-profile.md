...
## Video moments records

`skills.lov-video-moments.records.edit_method` stores only a directly stated
long-term choice: `photographic` or `imagegen`. A persistent photographic choice
is sufficient to proceed on later runs when the host permits deterministic
editing. Do not reconfirm the same method or run a generative sample first.
It is separate from `visual_style=bright-documentary`, which describes the
appearance and does not authorize a particular editing tool.

White-balance gains, exposure values, source paths and selection timestamps are
run-specific. Record them in the run's `edits.json`, not as universal defaults.
